...
Let the indicator function $\mathbb{1}(\cdot)$ be defined so that $\mathbb{1}(cond)=1$ if $cond$ is true, and $0$ otherwise. The basic Proximus algorithm~\cite{proximus} provides an approximation to the rank-one matrix $uv\transp$ that is closest to the given matrix $X$ in Hamming distance,
...
\Repeat {$u\iter{k}(v\iter{k})\transp = u\iter{k-1}(v\iter{k-1})\transp$} {
  $s\iter{k} \leftarrow Av\iter{k} $;\, $w\iter{k} \leftarrow \|v\iter{k}\|_0$  \;
  $u\iter{k+1}_i \leftarrow \bool(s\iter{k}_i > w\iter{k}/2),\;\forall\,i$ \;
  $t\iter{k+1} \leftarrow A\transp{u\iter{k+1}} $;\, $z\iter{k+1} \leftarrow \|u\iter{k+1}\|_0$  \;
  $v\iter{k+1}_j \leftarrow \bool(t\iter{k+1}_j > z\iter{k+1}/2),\;\forall\,j $ \;
  $k \leftarrow k+1 $ \;
}
\label{alg:proximus}
\end{algorithm}
...
The above algorithms, as all dictionary learning algorithnms, are  non-convex and very sensitive to initialization. Here we propose a number of initialization strategies, including and discussing the ones proposed for the Proximus method.

PENDIENTE: meter esta discusion mejor
