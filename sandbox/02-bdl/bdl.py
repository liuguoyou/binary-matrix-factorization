...
def bdl1(X,K,lu=0.0):
    ...
    for it in range(maxit):
        ...
        # 1) update V
        Hv = np.dot(U.T,U)
        Sv = np.diag(1. / np.maximum(np.diag(Hv),1.)) # 'diagonalized Hessian'
        #Gv = np.dot(U.T,X) - np.dot(Hv,V) + lv*V
        Ev = X - (np.dot(U,V) % 2)
        ...
        # 2) update U
        Hu = np.dot(V,V.T)
        Su = np.diag(1. / np.maximum(np.diag(Hu),1.))
        Eu = X - (np.dot(U,V) % 2)
        #Gu = np.dot(X,V.T) - np.dot(U,Hu)  + lv*U
        Gu = -np.dot(Eu,V.T)  + lv*U
        dU = np.dot(-Gu,Su)
        #print 'Eu=',Eu,'\nU=',U,'\nGu=',Gu,'\nHu=',Hu,'\nSu=',Su,'\ndU=',dU
        U  = binarize(U + dU)
        #print 'Vp=',Vp,'V=',V
